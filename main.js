...
core.on('message_edit', ($) => {
  $.text(`Я тут заметил, что ты изменил сообщение. Теперь оно смотрится вот так: ${$.msg}`);
});

core.on('no_match', ($) => {
  $.text("Я тебя не пойму! Напиши /help чтобы посмотреть список комманд.");
});

core.on('message_allow', ($) => {
  $.text('Ты со мной уже помирился?');
});

core.on('message_typing_state', ($) => {
  $.text('Пока ты что-что пишешь, я отвечу другим. ');
});
