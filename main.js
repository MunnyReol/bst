...
core.on('message_edit', ($) => {
  $.text(`Я только-что заметил, что ты изменил сообщение. Теперь оно выглядит так: ${$.msg}`);
});

core.on('no_match', ($) => {
  $.text("Я тебя не понимаю! Напиши /help для помощи.");
});

core.on('message_allow', ($) => {
  $.text('Ты уже со мной помирился?');
});

core.on('message_typing_state', ($) => {
  $.text('Пока ты что-то пишешь, я отвечу другим. ');
});
